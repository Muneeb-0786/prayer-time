...
export const PrayersProvider = ({ children }) => {
  ...
  const location = useMemo(() => [geolocation.latitude, geolocation.longitude], [geolocation.latitude, geolocation.longitude]);
         ...
         console.log(location,'location')
          const data = await axiosApi.get(
            `timings/${new Date().getDate()}-${
              new Date().getMonth() + 1
            }-${new Date().getFullYear()}?latitude=${location[0]}&longitude=${
              location[1]
            }&method=${method}`
          );
